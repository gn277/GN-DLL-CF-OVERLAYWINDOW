...
//换弹搜5，没换搜2
//主武器0，副武器1，近战2，雷3
#define InjectHookOffset 0x1B8A54								//dll注入点					特征码：45 33 C9 C7 44 24 ?? 01 00 00 00 45 33 C0 48 ?? ?? ?? 33 D2 FF 15 ?? ?? ?? ?? 85 C0 75 ?? E8 ?? ?? ?? ?? 85 C0
#define Mtrix 0x14051F4E0										//矩阵						特征码：F3 0F 10 83 ?? ?? ?? ?? F3 0F 10 8B ?? ?? ?? ?? 48 8B 4B ?? 48 85 C9 0F 84 ?? ?? ?? ?? 44 8B 81 ?? ?? ?? ??  [rbx + 0x2D00]-0x10
#define ArraySizeOffset 0x1040									//数组大小
#define PlayerArrayBaseAddress 0x2A349C8						//玩家数组
#define ModelOffset 0x2A34A00									//判断模式
#define ModelOffset2 0x2A349D9									//模式基址
...
#define Hitchaddress 0x1403B0E84								//绘制挂接地址				特征码：45 33 C9 45 33 C0 33 D2 E9 ?? ?? ?? ?? FF 15 ?? ?? ?? ?? 8B D8 
#define DrawIndexedPrimitiveaddress 0x140254CD7					//渲染挂接地址				特征码：44 89 64 24 20 FF 90 ?? ?? 00 00 83 3D ?? ?? ?? ?? 02 7C ?? 8B 84 24 ?? ?? ?? ?? 按顺序数第十个地址
#define SilentTrackAddress 0x1402314E0							//静默追踪构造				特征码：48 83 EC 28 0F 28 C2 4C 8B CA F3 0F 10 54 24 ?? 
...
#define RangeTrackAddressOne 0x17044D							//范围追踪1					特征码：F3 0F 11 52 ?? F3 0F 11 4A?? F3 0F 11 42 ?? F3 0F 10 92 ?? ?? ?? ?? F3 0F 10 8A ?? ?? ?? ?? F3 0F 10 82 ?? ?? ?? ?? F3 0F 58 92 ?? ?? ?? ?? F3 0F 58 8A ?? ?? ?? ?? F3 0F 58 82 ?? ?? ?? ?? F3 0F 11 52 ?? F3 0F 11 4A ?? F3 0F 11 42 ?? 0F B6 82 ?? ?? ?? ?? 
...
#define BarrierOffset 0x14004FB60								//障碍判断基址				特征码：4C 8B C9 4C 8B C2 48 8B ?? ?? ?? ?? ?? 49 8B D1 48 8B 01 48 FF ?? 18
#define KnifeDistanceBaseAddress 0x2BCF6D8						//刀距基址 | 武器基址
...
#define BiochemicalCampOffset 0x113FF							//生化阵营偏移 | 玩家阵营
#define BoneOffset 0x28E0										//骨骼偏移
